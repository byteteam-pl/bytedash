...
<body>
    ...
    <div class="box">
        <div class="container" style="height: 800px;">
            ...
            ?>
            <span><?php echo $Translate->getTranslate('welcome-subtext-register');?></span>
            <form action="/register" method="POST">
                <input autocomplete="username" type="email" name="email"
                    placeholder="<?php echo $Translate->getTranslate('email-form');?>" />
                <input type="password" name="password1"
                    placeholder="<?php echo $Translate->getTranslate('password');?>" />
                <input type="password" name="password2"
                    placeholder="<?php echo $Translate->getTranslate('repeat-password');?>" />
                <input type="hidden" name="byte_csrf" value="<?php echo HttpKernel::generateCSRF();?>">
                <div class="g-recaptcha" data-sitekey="<?php echo $config['captcha']['publicKey'];?>"></div>
                <button type="submit" class="btn-primary"><?php echo $Translate->getTranslate('register');?></button>
            </form>
        </div>
    </div>
